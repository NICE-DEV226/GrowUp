...
export default function Goals() {
  ...
  const loadGoals = async () => {
    try {
      ...
      if (error.response?.status === 401) {
        Alert.alert(t('sessionExpired'), t('pleaseReconnect'));
  ...
  const handleUpdateGoal = () => {
    if (!goalTitle.trim()) {
      Alert.alert(t('error'), t('enterGoalTitle'));
      return;
    }
    ...
    if (selectedGoal) {
      ...
      setTimeout(() => {
        Alert.alert(
          t('goalModified'), 
          t('goalModifiedMessage').replace('{title}', goalTitle),
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        Alert.alert(
          t('goalCreated'), 
          t('goalCreatedMessage').replace('{title}', goalTitle),
          [{ text: 'Super !', style: 'default' }]
        );
      }, 300);
    } catch (error: any) {
      console.error('Erreur création objectif:', error);
      Alert.alert(t('error'), error.response?.data?.error || t('cannotCreateGoal'));
    }
  };
    ...
    try {
      ...
      if (response.data.goal.isAchieved) {
        setTimeout(() => {
          Alert.alert(
            `🎉 ${t('congratulations')}`,
            `${t('goalAchieved')} "${selectedGoal.title}" !`,
        ...
        setTimeout(() => {
          Alert.alert(
            t('allocationSuccess'),
            t('allocationSuccessMessage')
              .replace('{amount}', amount.toString())
              .replace('{symbol}', currencySymbol)
              .replace('{title}', selectedGoal.title),
            [{ text: 'OK', style: 'default' }]
          );
        }, 300);
      }
      ...
      Alert.alert(t('error'), error.response?.data?.error || t('cannotAllocate'));
    }
  };
  ...
  const handleDeleteGoal = () => {
    ...
    Alert.alert(
      t('deleteGoal'),
      `${t('confirmDelete')} "${selectedGoal.title}" ?`,
      [
  ...
  const categories = [
    { name: t('savingsCategory'), icon: 'piggy-bank' },
    { name: t('travel'), icon: 'beach' },
    { name: t('technology'), icon: 'laptop' },
    { name: t('vehicle'), icon: 'car' },
    { name: t('realEstate'), icon: 'home' },
    { name: t('securityGoal'), icon: 'shield-check' },
    { name: t('giftGoal'), icon: 'gift' },
    { name: t('educationCategory'), icon: 'school' },
  ];
    ...
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.header, { opacity: headerOpacity }]}>
        <View style={styles.headerContent}>
          <View>
            <Text style={styles.headerTitle}>{t('myGoals')}</Text>
            <Text style={styles.headerSubtitle}>
              {goals.length} {goals.length > 1 ? t('goals_plural') : t('goal')} • {achievedGoals} {achievedGoals > 1 ? t('achievedGoals_plural') : t('achievedGoal')}
            </Text>
          </View>
          <TouchableOpacity 
      ...
      >
        <Animated.View style={[styles.statsCard, { opacity: goalsOpacity }]}>
          <View style={styles.statsGradient}>
            ...
            <View style={styles.statItem}>
              ...
              <Text style={styles.statValue} numberOfLines={1} adjustsFontSizeToFit>
                {formatSmartAmount(totalSaved, currency, 10)}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.statsGradient}>
            ...
            <View style={styles.statItem}>
              <Text style={styles.statLabel}>{t('totalGoalAmount')}</Text>
              <Text style={styles.statValue} numberOfLines={1} adjustsFontSizeToFit>
                {formatSmartAmount(totalTarget, currency, 10)}
              </Text>
            </View>
          </View>
        </Animated.View>
        ...
        {goals.length > 0 && (
          <Animated.View style={[styles.globalProgressCard, { opacity: goalsOpacity }]}>
            <View style={styles.globalProgressHeader}>
              <Text style={styles.globalProgressTitle}>{t('progress')}</Text>
          ...
          {goals.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              >
                ...
                <Text style={styles.emptyButtonText}>{t('createGoal')}</Text>
              </TouchableOpacity>
            </View>
          ) : (
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>{t('newGoal')}</Text>
            ...
            >
              ...
              {/* Titre */}
              <Text style={styles.inputLabel}>{t('titleRequired')}</Text>
              <TextInput
                style={styles.input}
                placeholder={t('exampleVacation')}
              ...
              {/* Montant */}
              <Text style={styles.inputLabel}>{t('targetAmountRequired')}</Text>
              <TextInput
                style={styles.input}
              ...
              {/* Date limite */}
              <Text style={styles.inputLabel}>{t('deadlineOptional')}</Text>
              <TouchableOpacity 
              ...
              {selectedDate && (
                <TouchableOpacity onPress={() => setSelectedDate(null)}>
                  <Text style={styles.clearDateText}>{t('clearDate')}</Text>
                </TouchableOpacity>
              )}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>{t('editGoal')}</Text>
            ...
            {selectedGoal && (
              <>
                ...
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>{t('saved')}:</Text>
                ...
                {!selectedGoal.isAchieved && (
                  <TouchableOpacity style={styles.allocateBtn} onPress={openAllocate}>
                    <Text style={styles.allocateBtnText}>{t('allocateMoney')}</Text>
                  </TouchableOpacity>
                )}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <Text style={styles.modalTitle}>{t('allocateMoney')}</Text>
            ...
            {selectedGoal && (
              <>
                ...
                <TextInput
                  style={styles.input}
                  placeholder={`${t('allocateAmount')} (${currencySymbol})`}
                  placeholderTextColor="rgba(253, 253, 253, 0.4)"
                  keyboardType="decimal-pad"
          ...
          <View style={styles.calendarModal}>
            <View style={styles.calendarHeader}>
              ...
              <Text style={styles.calendarTitle}>
                {formatDateUtil(calendarMonth, language, { month: 'long', year: 'numeric' })}
              </Text>
